/* 
   ...
   - reverseGeocodeDisplay() / renderLocationAsync()
                        display-only lat/lng -> address conversion,
...
/* =====================================================================
   Reverse geocoding — DISPLAY ONLY.
   Converts a lat/lng pair into a human-readable address purely for
   showing in the UI. This NEVER writes anything back to Supabase —
   the sos_alerts table's lat/lng columns are untouched and remain the
   source of truth for the map, distance calculations, and nearest-
   hospital search. Results are cached in-memory (per page load) and
   requests are queued one-at-a-time with a short delay, in line with
   Nominatim's usage policy (no parallel bulk requests).
===================================================================== */
const _mhGeoCache = new Map();          // "lat,lng" (5dp) -> address string | null
let _mhGeoQueueTail = Promise.resolve(); // serializes outgoing requests

function _mhGeoKey(lat, lng) {
  return `${Number(lat).toFixed(5)},${Number(lng).toFixed(5)}`;
}

async function reverseGeocodeDisplay(lat, lng) {
  if (lat == null || lng == null) return null;
  const key = _mhGeoKey(lat, lng);
  if (_mhGeoCache.has(key)) return _mhGeoCache.get(key);

  const run = _mhGeoQueueTail.then(async () => {
    ...
    try {
      ...
      _mhGeoCache.set(key, label);
      return label;
    } catch (e) {
      console.warn("[MediHelp Admin] reverse geocode failed for", key, e);
      _mhGeoCache.set(key, null); // cache the miss too, so a slow/blocked call isn't retried on every re-render
      return null;
    } finally {
      clearTimeout(timeout);
      await new Promise((r) => setTimeout(r, 1000)); // ~1 request/sec pacing
    }
  });
  _mhGeoQueueTail = run;
  return run;
...
/**
 * Fills a table/list cell with a location value. Shows the raw
 * coordinates immediately (so the table never appears blank or
 * blocked), then swaps in a human-readable address once available —
 * either instantly if the row already has one (e.g. sos-monitor.js
 * stored it at insert time) or asynchronously via reverse geocoding.
 * On failure, shows "Location unavailable" instead of breaking anything.
 */
function renderLocationAsync(cellEl, row) {
  if (!cellEl) return;
  if (row.lat == null || row.lng == null) { cellEl.textContent = "—"; return; }

  if (row.address) {
    cellEl.textContent = row.address;
    return;
  }

  cellEl.textContent = `${Number(row.lat).toFixed(5)}, ${Number(row.lng).toFixed(5)}`;
  reverseGeocodeDisplay(row.lat, row.lng)
    .then((label) => { cellEl.textContent = label || "Location unavailable"; })
    .catch(() => { cellEl.textContent = "Location unavailable"; });
}
 ...
 *   emptyMessage: string,
 *   afterRender?: (rows, tableEl) => void   // optional; runs after each render, e.g. for async cell fill-ins
 * }
 */
class CrudManager {
  constructor(config) {
  ...
  render() {
    const { columns, tableEl, emptyMessage, afterRender } = this.config;
    tableEl.innerHTML = "";
    ...
    // Optional hook for pages that need to do async work per rendered row
    // (e.g. filling in reverse-geocoded addresses) without touching the
    // core render path used by every other page.
    if (typeof afterRender === "function") {
      afterRender(this.rows, tableEl);
    }
  }
